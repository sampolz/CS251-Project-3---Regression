...
class Data:
    ...
    def __str__(self):
        ...
        output = "-------------------------------\n"
        output += f"{self.filepath} ({num_rows}x{num_columns})\n"
        output += "Headers:\n  "
        output += "   ".join(self.headers) + "\n"
        output += "-------------------------------\n"
        output += f"Showing first 5/{num_rows} rows.\n"

        # Show the first five rows of self.data
    ...
    def select_data(self, headers, rows=[]):
        ...
        '''
        toReturn = []
        if not isinstance(headers, list): # if a string is input, converts to a list
            headers = [headers]
        if(len(rows) == 0):
